...
Stage2_1::Stage2_1() : Scene(L"Stage2_1")
{
...
#pragma region 플레이어 오브젝트
	GameObject* playerobject = GameObjectFactory::GetInstance().CreatePlayer(1.0f, true, Code404::Layer::Player,Vector2D(560, 715), Vector2D(560 + 1920 * 0, 715 + 1080 * 0), true);
	playerobject->GetTransform()->SetLocalPosition(Vector2D(-300.0f, 970.f));
#pragma endregion

#pragma region 플랫폼
	Level level(STAGE_TWO_ONE);
	level.Tiling(level);
...
#pragma region 다음 씬 로드
	...
	outBlackMask->SetScene(L"Stage2_2");
...
#pragma region 배경
	{
		...
		background->GetTransform()->SetLocalPosition(Vector2D(462.f, 715.f)); 
	...
	// {
	// 	GameObject* background = CreateGameObject(GetMainCamera()->GetGameObject(), L"BackGround");
	// 	background->GetTransform()->SetLocalPosition(Vector2D(200.f, 100.f));
	// 	SpriteRenderer* sprite = background->AddComponent<SpriteRenderer>();
	...
	// {
	// 	GameObject* background = CreateGameObject(GetMainCamera()->GetGameObject(), L"BackGround");
	// 	background->GetTransform()->SetLocalPosition(Vector2D(0.f, 350.f));
	// 	SpriteRenderer* sprite = background->AddComponent<SpriteRenderer>();
